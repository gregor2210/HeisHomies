...
func main() {
	...
	TCP_receive_channel := make(chan connectivity.Worldview_package, 1)
	TCP_send_channel_listen := make(chan connectivity.Worldview_package, 1)
	TCP_send_channel_dail := make(chan connectivity.Worldview_package, 1)
	connectivity.TCP_setup(TCP_receive_channel, TCP_send_channel_listen, TCP_send_channel_dail)
	...
	for {
		...
		case <-world_view_send_ticker:
			...
			connectivity.Send_elevator_world_view(TCP_send_channel_listen, TCP_send_channel_dail)
