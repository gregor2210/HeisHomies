...
func main() {
	...
	tcpReceiveChannel := make(chan connectivity.WorldviewPackage, 1)
	TCP_send_channel_listen := make(chan connectivity.WorldviewPackage, 1)
	TCP_send_channel_dail := make(chan connectivity.WorldviewPackage, 1)
	connectivity.TCP_setup(tcpReceiveChannel, TCP_send_channel_listen, TCP_send_channel_dail)
	...
	for {
		...
		case <-worldViewSendTicker:
			...
			connectivity.Send_elevator_worldView(TCP_send_channel_listen, TCP_send_channel_dail)
