...
func main() {
	...
	worldView_resever_chan := make(chan connectivity.WorldviewPackage) // WORLD VIEW IS ONLY A STRING TEMPORALRY
	go connectivity.Receive_elevator_worldView_distributor(worldView_resever_chan)
	...
	for {
		...
		case worldView := <-worldView_resever_chan:
			...
			if master {
				...
				//Update the world view
				//If gotten a new order from master, -> fsm.FsmOnRequestButtonPress(a.Floor, a.Button)
			}

			//fsm.PrintElevator(worldView)

		case <-worldViewSendTicker:
			connectivity.Send_elevator_worldView()
