...
func SerializeElevator(wv masterSlave.WorldviewPackage) ([]byte, error) {
...
func DeserializeElevator(data []byte) (masterSlave.WorldviewPackage, error) {
	var wv masterSlave.WorldviewPackage
...
func TcpReceivingSetup(tcpReceiveChannel chan masterSlave.WorldviewPackage, TCP_send_channel_listen chan masterSlave.WorldviewPackage, TCP_send_channel_dail chan masterSlave.WorldviewPackage) {
...
func handleReceive(conn net.Conn, tcpReceiveChannel chan masterSlave.WorldviewPackage, connectedElevatorID int, conn_type string) {
	defer conn.Close()
	if conn_type == "server" {
...
func SendWorldView() {
	SendWorldviewPackage := masterSlave.NewWorldviewPackage(ID, fsm.GetElevatorStruct())
